...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), n_landmarks=5):  # detection layer
        super(Detect, self).__init__()
        self.nc = nc  # number of classes
        #self.no = nc + 5  # number of outputs per anchor
        self.n_landmarks = n_landmarks
        self.no = nc + 5 + (self.n_landmarks * 2)  # number of outputs per anchor
    ...
    def forward(self, x):
        ...
        if self.export_cat:
            for i in range(self.nl):
                ...
                y = y + torch.cat((x[i][:, :, :, :, 0:5].sigmoid(), torch.cat((x[i][:, :, :, :, 5:5+self.n_landmarks*2], x[i][:, :, :, :,  5+self.n_landmarks*2: 5+self.n_landmarks*2+self.nc].sigmoid()), 4)), 4)
                ...
                landm = {}
                idx = 5
                for j in range(1, self.n_landmarks + 1):
                    landm[j] = y[:, :, :, :, idx:idx+2] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]
                    idx = idx + 2
                ...
                y = torch.cat([box_xy, box_wh, y[:, :, :, :, 4:5], *[landm[j] for j in range(1, self.n_landmarks+1)], y[:, :, :, :, 5+self.n_landmarks*2:5+self.n_landmarks*2+self.nc]], -1)
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = torch.full_like(x[i], 0)
                class_range = list(range(5)) + list(range(5+self.n_landmarks*2,5+self.n_landmarks*2+self.nc))
                y[..., class_range] = x[i][..., class_range].sigmoid()
                y[..., 5:5+self.n_landmarks*2] = x[i][..., 5:5+self.n_landmarks*2]
                #y = x[i].sigmoid()
                ...
                idx = 5
                for j in range(1, self.n_landmarks + 1):
                    y[..., idx:idx+2]   = y[..., idx:idx+2]  * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i] # landmark x1 y1
                    idx = idx + 2
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, n_landmarks=5):  # model, input channels, number of classes
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], n_landmarks=n_landmarks)  # model, savelist
...
def parse_model(d, ch, n_landmarks=5):  # model_dict, input_channels(3)
